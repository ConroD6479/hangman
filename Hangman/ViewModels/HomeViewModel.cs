...
namespace Hangman.ViewModels
{
    public class HomeViewModel : NotifyViewModel
    {
        ...
        public void LetterPressed(object buttonClicked)
        {
            ...
            if (!Regex.Match(WordOnDisplay, "＿").Success)
            {
                ...
                if (user.GameProperty.LevelProperty >= 5)
                {
                    ...
                    serializationActions.SerializeUsers(Constants.UsersFile, users);
                    ShowMessageBox("Вы победили", "Вы победили в категории " + user.GameProperty.CategoryProperty.ToString() + ".\nХотите ли вы начать новую игру в этой же категории?", MessageBoxImage.Information);
                    return;
                }
                ReloadGame();
            }
            ...
            else
            {
                ...
                serializationActions.SerializeUsers(Constants.UsersFile, users);
                HangImageSource = images.Hangs[++user.GameProperty.MistakesProperty];
                ShowMessageBox("Вы проиграли", "Вы проиграли игру в категории " + user.GameProperty.CategoryProperty.ToString() + ".\nХотите ли вы начать новую игру в этой же категории?", MessageBoxImage.Error);
            }
        }
        ...
        private void TimeExpired()
        {
            ...
            serializationActions.SerializeUsers(Constants.UsersFile, users);
            ShowMessageBox("Время вышло", "Время вышло!\nХотите ли вы начать новую игру в этой же категории?", MessageBoxImage.Warning);
        }
        ...
        public void NewPressed(object param)
        {
            int seconds = (deadline - DateTime.Now).Seconds;
            StopTimer();
            MessageBoxResult messageBoxResult = MessageBox.Show("Если вы начнете новую игру, в текущей будет засчитано поражение.\nВы действительно хотите начать новую игру?", "Новая игра", MessageBoxButton.YesNo, MessageBoxImage.Question);
            if (messageBoxResult == MessageBoxResult.Yes)
            {
                user.GameProperty.MistakesProperty = 0;
                user.GameProperty.LevelProperty = 1;
        ...
        public void SelectCategory(object menuItem)
        {
            if ((menuItem as MenuItem).Header.ToString() != CategoryLabel.ToString())
            {
                int seconds = (deadline - DateTime.Now).Seconds;
                StopTimer();
                MessageBoxResult messageBoxResult = MessageBox.Show("Вы действительно хотите изменить категорию?\nЕсли вы ее измените, в этой игре будет засчитано поражение!", "Изменить категорию?", MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (messageBoxResult == MessageBoxResult.Yes)
                {
                    switch (oldCategory)
                    {
